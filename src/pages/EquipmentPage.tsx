...
import { Plus, Trash2, Truck, Settings2 } from "lucide-react";
...
export default function EquipmentPage() {
  const [equipments, setEquipments] = useState(store.getEquipments());
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ name: '', type: 'machine' as Equipment['type'], plate: '', model: '', fuelCapacity: '', currentFuel: '' });

  const refresh = () => setEquipments(store.getEquipments());
  ...
  const handleSave = () => {
    const eq: Equipment = {
      ...
      currentFuel: form.currentFuel ? Number(form.currentFuel) : undefined,
      status: 'active',
          ...
          <DialogContent className="bg-card border-border">
            ...
            <div className="space-y-4">
              ...
              <div><Label>Tipo *</Label>
                <Select value={form.type} onValueChange={v => setForm({...form, type: v as Equipment['type']})}>
            ...
            <div key={eq.id} className="glass-card rounded-xl p-5 hover:border-primary/30 transition-all group">
              ...
              <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg font-mono font-bold text-primary">{eq.currentHourMeter}h</p>
                </div>
                {eq.type === 'combo' && eq.fuelCapacity && (
                  <div className="text-right">
                    <p className="text-xs text-muted-foreground">Combustível</p>
                    <p className="text-lg font-mono font-bold text-accent">{eq.currentFuel || 0}L / {eq.fuelCapacity}L</p>
                  </div>
                )}
              </div>
            </div>
